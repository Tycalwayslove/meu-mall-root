...
| Java H5 版本管理 | active manifest、candidate 注册、版本列表、promote、gray、rollback |
| 根目录 `scripts/register-resolver/` | 固定 `/register` 入口解析到当前 active H5 注册页 |
| 根目录 `scripts/deploy/h5-version-deploy.sh` | 真正执行 H5 多版本部署的核心脚本 |

旧 `server-meumall`、`admin-meumall`、`app-meumall` 和 `meumall-ci` 已退役/外部化，并已从当前工作区移除，不再作为当前发版对象或实现范围。
...
```text
{JAVA_H5_RELEASE_API_BASE_URL}/platform/h5Release/active
```
...
| 服务端 | Java H5 版本管理 active 接口可访问，例如 `https://test.aigcpop.com:18088/apis/platform/h5Release/active` |
...
curl -kfsSL "${JAVA_H5_RELEASE_API_BASE_URL%/}/platform/h5Release/active"
```
...
```bash
curl -kfsSL "${JAVA_H5_RELEASE_REGISTER_API_BASE_URL%/}/platform/h5Release/list"
```
...
5. 校验 H5 工作区必须干净。
6. 请求 Java H5 版本管理 active manifest（`/platform/h5Release/active`），读取当前 `stableVersion` 作为新版本的 `rollbackVersion`。脚本不再允许用旧 Python `/api/h5/manifest/active` 或 Java 业务 `/mini_h5` 作为版本管理来源。
...
12. 启动注册公开入口 Node resolver：
    - 容器名：`meu-mall-register-resolver`
    - 默认监听：`127.0.0.1:4110`
    - Java active manifest 前缀来自 `JAVA_H5_RELEASE_API_BASE_URL`
13. 写入 Nginx 版本 location 和 `/register` 精确代理：
    - `/opt/mail4j/nginx/conf.d/meu-mall-h5-versions/vX.Y.Z.conf`
14. reload `mall4j-nginx`。
15. 远端 smoke：
...
16. 调用 `hybird-meumall/scripts/ai/register-release.ts` 生成 release payload。
17. 请求 Java `POST /platform/h5Release` 注册 candidate release。
18. 如果 `PROMOTE_RELEASE=true`，继续请求 Java `POST /platform/h5Release/{id}/promote` 切 active。
19. 如果 `SEND_FEISHU_REVIEW=true`，执行 `pnpm run feishu:h5-release-notice -- request-review`，并把 `H5_RELEASE_ENV`、`JAVA_H5_RELEASE_API_BASE_URL`、`JAVA_H5_RELEASE_REGISTER_API_BASE_URL` 等 Java release 配置显式传给飞书通知脚本；通知脚本会从 Java active/list 取当前版本和 commit。
...
| `INSTALL_REGISTER_RESOLVER` | `true` | 是否部署并启动 `/register` Node resolver |
| `REGISTER_RESOLVER_PORT` | `4110` | `/register` Node resolver 的本机监听端口，Nginx 安装时会同步替换 |
| `REGISTER_RESOLVER_CONTAINER` | `meu-mall-register-resolver` | `/register` Node resolver 容器名 |
...
当前轻量流程下，Jenkins 负责测试环境 H5 版本构建、部署、smoke、Git tag 和 Java candidate 注册；版本 promote、gray 和 rollback 由外部 Java H5 版本管理能力承接。
...
```text
GET  /platform/h5Release/active
GET  /platform/h5Release/list
...
当前仓库不再维护本地 Jenkins controller、Mac agent 或 `meumall-ci` 工作区。外部 Jenkins 如果调用仓库脚本，应直接执行：
...
- 测试服务器 IP、用户、端口、目录、域名和 SSH 密码或 SSH key。
