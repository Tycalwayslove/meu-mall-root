...
| `server-meumall` | release 注册、active manifest、灰度、回滚 API |
| `admin-meumall` | 可视化发布、灰度、回滚管理台 |
...
App 不应该写死某个 H5 URL，而是先读取 active manifest：
...
| 服务端 | `https://hybird.aigcpop.com/api/h5/manifest/active?environment=prod` 可访问 |
...
curl -kfsSL https://hybird.aigcpop.com/api/h5/manifest/active?environment=prod
```
...
```bash
curl -kfsSL https://hybird.aigcpop.com/api/releases?environment=prod
```

找到目标版本对应的 `id` 后执行：

```bash
curl -kfsSL -X POST https://hybird.aigcpop.com/api/releases/{releaseId}/promote
```
...
5. 校验 H5 工作区必须干净。
6. 请求线上 active manifest，读取当前 `stableVersion` 作为新版本的 `rollbackVersion`。
...
18. 如果 `SEND_FEISHU_REVIEW=true`，执行 `pnpm run feishu:h5-release-notice -- request-review`，把本版本待审核通报发送到飞书审核群。
...
Jenkins 只是把同一个脚本包装成可视化任务，本质仍然执行：
...
| 参数 | 建议 |
| --- | --- |
| `GIT_REF` | 可以留空；留空时使用 `h5/v{hybird-meumall/package.json version}` |
| `REMOTE_HOST` | `8.163.107.208` |
| `REMOTE_USER` | `root` |
| `REMOTE_PORT` | `22` |
| `REMOTE_PATH` | `/opt/mail4j/meu-mall` |
| `DOMAIN` | `hybird.aigcpop.com` |
| `H5_HOST_PORT` | 留空，自动选择 |
| `NEXT_PUBLIC_H5_ASSET_BASE_URL` | 当前留空 |
| `SERVER_PASSWORD` | 填服务器 SSH 密码 |
| `REGISTER_RELEASE` | 勾选 |
| `PROMOTE_RELEASE` | 想直接切 active 就勾选；只发 candidate 就不勾选 |
| `INSTALL_NGINX` | 勾选 |
| `RUN_REMOTE_SMOKE` | 勾选 |

Jenkins 发版前仍然要先完成本地 H5 commit、tag、push。Jenkins 不负责帮你自动改版本号或创建 tag。
...
curl -kfsSL \
  ...
  https://hybird.aigcpop.com/api/releases/{releaseId}/rollback
```
...
- 或调用 `POST /api/releases/{id}/promote`。
