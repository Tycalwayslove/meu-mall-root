...
H5 继续在版本容器内承载 `/register` 页面。对外只暴露固定入口 `https://hybird.aigcpop.com/register`，由独立 Node resolver 解析 Java 当前 active manifest 并跳转到当前生效版本的注册页。

## 页面范围

| 页面 | 路由 | 端归属 | 说明 |
| --- | --- | --- | --- |
| 注册页公开入口 | `/register` | Node register resolver | 302 到 active H5 版本注册页 |
| 注册页实际页面 | `/h5-v/<version>/register` | H5 | 由 hybird-meumall 版本容器承载 |

## 数据流

```text
用户扫码 -> GET /register -> Node resolver 查询 Java active manifest -> 302 /h5-v/<version>/register -> H5 注册页
```
...
| 事项 | 是否需要 | 说明 | 契约 |
| --- | --- | --- | --- |
| 新增接口 | 是 | Node resolver 提供 `GET /register`，返回 302 | `.ai-workspace/contracts/api/h5-register-public-entry-contract.md` |
| 调整接口 | 是 | Java `GET /platform/h5Release/active` 当前不再需要 `environment` query | 同上 |
| 鉴权 | 否 | 公开二维码入口无需鉴权；resolver 可通过 `JAVA_H5_RELEASE_TOKEN` 访问 Java active 接口 | 同上 |
...
### Node register resolver / CI

- [x] 提供 `GET /register` 固定入口。
- [x] 查询 Java active manifest 并生成 302 目标。
- [x] active manifest 未声明 `/register` 时返回 404。
- [x] Jenkins 发版脚本同步 resolver 源码并启动容器。

### 运维

- [x] nginx 模板将公网 `/register` 转发到 Node resolver。
...
- Java active manifest 请求失败或格式异常：`GET /register` 返回 502。
...
该入口由 Node resolver 查询 Java active manifest 后 302 到当前 H5 版本：
https://hybird.aigcpop.com/h5-v/<version>/register

Jenkins 会启动 Node resolver 容器；nginx 将公网 /register 转发到该 resolver。运营二维码不要再使用 /h5-v/<version>/register。
```
...
| 2026-07-06 | 需求方 | 已确认 | Java 不新增入口，改由 Node resolver 调用不带 environment query 的 Java active manifest |
| 2026-07-06 | 飞书知识库 | 已同步 | 规则页 revision 11；页面盘点 revision 73；H5 发版流程 revision 7 |
