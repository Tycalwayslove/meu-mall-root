...
当前测试环境 `paySettlementType=1`，微信支付不走普通微信 App 支付参数，而是由 Java `/p/order/pay` 返回通联小程序收银台参数。H5 会把 Java `/p/order/pay` 完整 `data` 放入 Native Bridge `paymentStartWechat.sdkPayload`；当 `data.result == 0` 且 `data.chnlFrontParamInfo` 可解析时，H5 会把该 JSON 字符串解析为 `paymentStartWechat.chnlFrontParamInfo` 并将对象内所有顶层参数传给原生，同时保留 `paymentMode=allinpay-mini-program-bridge` 和历史兼容字段 `miniProgram`。原生 App 当前联调方案以 `sdkPayload/chnlFrontParamInfo` 为准，直接打开通联微信小程序收银台；旧 App 若仅支持 `paymentStartCashier`，H5 会在 `unsupported` 时自动 fallback。
...
sequenceDiagram
  ...
  H5->>App: rpc/paymentStartWechat(provider=allinpay, sdkPayload, chnlFrontParamInfo, miniProgram)
  App->>TL: 按 sdkPayload/chnlFrontParamInfo 直开通联收银台
  H5-->>U: replace 到 /pay-result
  U->>App: 支付后回到 App
  H5->>BFF: GET /api/bff/order-is-paid?payEntry=0&orderNumbers=...
  BFF->>Java: GET /p/order/isPay/0/{orderNumbers}
  Java-->>BFF: data=true/false
  BFF-->>H5: paid/unpaid
  H5-->>U: 展示最终或待确认状态
```
...
| `miniProgram.*` | 历史兼容辅助字段，保留 `allinpayParams`、订单号和通联收银台 appId；当前原生新实现不依赖喵呜小程序桥页。 |

## 原生实现要求

- `paymentStartWechat` 收到 `paymentMode=allinpay-mini-program-bridge` 时，不按普通微信支付 SDK 参数解析；iOS / Android 均按 `sdkPayload/chnlFrontParamInfo` 直接打开通联小程序收银台。
- App 不重新改写 `sdkPayload`、`chnlFrontParamInfo` 或 `miniProgram.extraData.allinpayParams` 内字段；需要完整支付返回时读取 `sdkPayload`，需要通联前置参数对象时读取 `chnlFrontParamInfo`。
...
- H5 发出 `paymentStartWechat` 后立即 `replace` 到 `/pay-result?sts=pending&orderNumbers=<orderNumbers>`，不等待 App 返回最终支付状态。
- `/pay-result` 自动调用 `/api/bff/order-is-paid?payEntry=0&orderNumbers=<orderNumbers>`，用户点击“查看支付状态”也调用同一接口。
- `data=true` 展示支付成功；`data=false` 展示暂未支付成功；页面另提供“查看订单”。
...
- [ ] 原生收到 `paymentStartWechat`，payload 中 `sdkPayload` 为 `/p/order/pay` 完整 `data`，`chnlFrontParamInfo` 为解析后的对象。
- [ ] 原生按 `sdkPayload/chnlFrontParamInfo` 直开通联小程序收银台。
- [ ] H5 发起 Bridge 后立即 replace 到 `/pay-result`，不白屏。
- [ ] H5 进入 `/pay-result` 并调用 `/api/bff/order-is-paid?payEntry=0&orderNumbers=<orderNumbers>` 回查。
- [ ] Java `/p/order/isPay/0/{orderNumbers}` 返回 `data=true` 时，H5 展示支付成功；返回 `data=false` 时展示暂未支付成功。

## 风险

- 微信小程序收银台打开成功不一定能拿到最终支付结果，首期以 H5 回查为准。
- App 直开通联小程序收银台时，完整 `chnlFrontParamInfo` 需要作为对象原样使用，不枚举、不重组、不补签。
- 如果通联收银台 appId、启动参数或回跳策略变更，需要同时更新 H5 契约、App 配置和飞书知识库。
