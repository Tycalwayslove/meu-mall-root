...
| L-023 | 推广首页头像/昵称/徽章 -> 无跳转 | 点击头像/昵称/徽章 | H5 | 无 | 不发起跳转 | 无 | 无 | 无 | 当前错误，待删除 |
...
| L-037 | 我的 `/mine` -> 设置 Native | 点击设置 | H5 | App | 调用待扩展 `bridge.navigate({ route: "native_page", params: { name: "settings" } })` | 打开原生设置页 | Bridge，待扩展 | `native-page` | 当前错误占位 |
| L-038 | 二级 WebView -> 来源 Tab 根页面 | 点击导航返回且无 H5 history | H5 | App | 调用待扩展 `bridge.navigate({ route: "close_webview" })` 或 `back` | 关闭当前 WebView，露出来源 Tab 根 WebView | Bridge `router/navigate` | `close-current-webview` | 待扩展 |
...
当前已在类型里声明的 `navigate.route`：

```ts
type BridgeRoute = "home" | "back" | "product_detail" | "webview";
```
...
- `tab/close_webview/native_page` 是为了完成“二级页回根 Tab、关闭当前 WebView、打开原生设置”等正式链路，需要 H5 和 App 一起补齐的 route。
...
| --- | --- | --- | --- |
| 切 Tab 并关闭当前 WebView | 待 H5 扩展：`bridge.navigate({ route: "tab", params: { tab, closeCurrentWebView: true } })` | 切到 `home/promotion/mine` 对应原生 Tab；关闭当前二级 WebView。 | 用于二级页返回根 Tab，避免 H5 在当前 WebView 内 `router.push("/")` |
| 关闭当前 WebView | 待 H5 扩展：`bridge.navigate({ route: "close_webview" })` | 直接关闭当前二级 WebView。 | 比 `back` 更明确 |
| 打开原生设置 | 待 H5 扩展：`bridge.navigate({ route: "native_page", params: { name: "settings" } })` | 打开 App 设置页。 | 我的页设置入口使用 |
| H5 路由变化上报 | 待 H5 扩展：`bridge.emit("route_changed", payload)` | 记录当前 H5 页面标题、路径、是否可返回、fallback tab。 | 用于原生导航栏和手势返回 |
...
final class MeuMallBridgeHandler: NSObject, WKScriptMessageHandler {
    ...
    private func handleRouter(action: String, payload: [String: Any]?) {
        ...
        case "close_webview":
            navigator?.closeCurrentWebView()
        case "native_page":
            navigator?.openNativePage(params: params)
        default:
            break
        }
    }
