...
- 2026-06-09 iOS 调试壳已能消费 `webview`、`tab`、`back`、`close_webview`、`native_page` 和 `route_changed`；设置、分享、登录重认证仍是占位或待正式实现。
...
| L-037 | 我的 `/mine` -> 设置 Native | 点击设置 | H5 | App | H5 调用 `HybridLink strategy="native-page" nativePage="settings"`，底层发送 `router/navigate route=native_page` | 打开原生设置页 | Bridge `router/navigate` | `native-page` | iOS 调试壳为占位页，正式设置页待 App 实现 |
...
| 打开原生设置 | H5 已支持：`HybridLink strategy="native-page" nativePage="settings"`，底层发送 `route=native_page` | 打开 App 设置页。 | 我的页设置入口使用 |
