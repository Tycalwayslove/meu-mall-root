...
| 调整接口 | 否 | 通联微信使用 `/p/order/pay` 返回的 `miniprogramPayInfo_VSP` 等小程序支付字段，H5 透传给 App。 | 同上 |
...
| 事项 | 是否需要 | 说明 | 契约 |
| --- | --- | --- | --- |
| Native Bridge | 是 | 新增 `payment.pay`、`payment.openUrl` RPC；`payment.pay` 增加 `paymentMode=wechat-mini-program` 通联微信分支。 | `.ai-workspace/contracts/native-bridge/h5-payment-bridge-contract.md` |
...
- [ ] 保持 `/p/order/pay` 在 `paySettlementType=1 + payType=8` 返回 `miniprogramPayInfo_VSP` 或同义通联小程序支付字段；H5 会生成微信小程序 path。
- [ ] 确认 `/p/allinpay/order/getOrderStatus` 状态码。

### 原生 App

- [ ] 实现 `rpc/payment.pay` 普通分支调支付宝/微信 SDK。
- [ ] 实现 `rpc/payment.pay` 通联微信分支：`provider=allinpay`、`paymentMode=wechat-mini-program` 时使用微信 OpenSDK 打开 `miniProgram.originalId=gh_e64a1a89a0ad` 和 `miniProgram.path`。
...
- 通联微信小程序收银台打开后状态未知：App 返回 `unknown`，H5 携带 `bizOrderNo` 进入支付结果页并回查。
...
1. rpc/payment.pay：普通支付调支付宝/微信 SDK；通联微信支付在 `provider=allinpay`、`paymentMode=wechat-mini-program` 时打开微信小程序收银台。
...
联调方式：
创建待支付订单进入 /pay-way，点击支付宝/微信，H5 发起 BFF 支付申请。支付宝通联分支通过 payment.openUrl 打开 URL；微信通联分支通过 payment.pay 打开微信小程序收银台；H5 最终回查订单状态进入 /pay-result。
```
