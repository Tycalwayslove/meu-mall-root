...
H5 商品详情和订单确认已能创建待支付订单，但收银台当前只展示支付信息，不调用 `/p/order/pay`。测试环境已启用通联 `paySettlementType=1`，需要同时跑通普通 App 支付契约和通联主链路。

## H5 侧目标

用户在 `/pay-way` 选择支付宝或微信后，H5 调 BFF 申请支付参数，再通过 Native Bridge 交给 App 调 SDK 或打开通联外部 URL。支付后 H5 回查订单状态并进入 `/pay-result`。
...
-> Java /p/order/pay
-> 普通支付 Native SDK 或通联 getAliAppPayUrl/openURL
...
| 事项 | 是否需要 | 说明 | 契约 |
| --- | --- | --- | --- |
| Native Bridge | 是 | 新增 `payment.pay`、`payment.openUrl` RPC。 | `.ai-workspace/contracts/native-bridge/h5-payment-bridge-contract.md` |
...
- [ ] 确认 `/p/order/pay` 在 `paySettlementType=1 + payType=8` 的返回字段和推荐唤起方式。
...
```text
本次 H5 需求需要后端确认 /p/order/pay 的通联支付宝和通联微信返回字段，并需要 App 接入两个 Bridge：
1. rpc/payment.pay：App 调支付宝/微信 SDK。
2. rpc/payment.openUrl：App 打开通联或支付宝外部 URL。
...
联调方式：
创建待支付订单进入 /pay-way，点击支付宝/微信，H5 发起 BFF 支付申请，App 执行 Bridge，H5 回查订单状态进入 /pay-result。
```
