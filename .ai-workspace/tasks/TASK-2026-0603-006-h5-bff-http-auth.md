...
为 `hybird-meumall` 建立一套可长期复用的 HTTP 请求体系：原生 App 通过 Cookie 将登录态传给 H5，Next SSR / BFF 从 Cookie 读取 token，再按后端约定写入 `Authorization` header 调用 Python / Java 后端。当前 Python 使用 `Authorization: Bearer <pythonToken>`，Java / mall 使用 `Authorization: <mallToken>`。
...
- 继续按 `Authorization` 鉴权；Python 使用 `Bearer` 前缀，Java / mall 不拼接 `Bearer`。
...
- [x] 服务端 backend client 调用后端时会注入 `Authorization`；Python 为 `Bearer <pythonToken>`，Java / mall 为裸 `mallToken`。
