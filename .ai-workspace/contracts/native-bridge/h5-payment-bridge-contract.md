...
H5 运行在 App WebView 中，不能直接使用 uni-app 的 `uni.requestPayment`。H5 负责向 Java 申请支付参数，App 负责调支付宝/微信 SDK、打开通联支付宝外部 URL，或按通联微信支付参数直接打开通联小程序收银台。

## Bridge 方法一：`paymentStartAlipay` / `paymentStartWechat`

- 调用方向：H5 -> Native
- 信封：`{ module: "rpc", action: "paymentStartAlipay" | "paymentStartWechat", payload, callbackId }`
...
- 兼容策略：旧 App 若尚未实现专属 action，可返回 `unsupported`；H5 会 fallback 到旧 action `paymentStartCashier`，payload 不变。
...
| `paymentMode` | `"app-sdk" \| "allinpay-mini-program-bridge" \| "allinpay-url"` | 否 | 支付执行方式；缺省按 `app-sdk` 兼容。 | 新增模式需更新契约。 |
...
| `paymentUrl` | string | 否 | `paymentMode=allinpay-url` 时提供，由 H5 通过 `/p/allinpay/order/getAliAppPayUrl` 换取。 | App 应校验 URL scheme/host 白名单。 |
| `chnlFrontParamInfo` | object | 否 | `sdkPayload.chnlFrontParamInfo` 可解析时，H5 解析后的对象；对象内所有顶层参数原样传给原生。 | 通联微信 `result=0` 时优先提供。 |
| `miniProgram` | object | 否 | 通联微信历史兼容辅助字段；当前原生新实现以 `sdkPayload/chnlFrontParamInfo` 直开通联小程序收银台。 | 旧 App 不认识该字段时应返回 `unsupported`。 |
| `status` | string | 是 | `success/cancelled/failed/unknown`。 | H5 仍需回查订单。 |

### 支付宝分支

H5 在用户选择支付宝 `payType=7` 时调用 `paymentStartAlipay`。普通支付宝支付使用 `paymentMode=app-sdk`；通联支付宝使用 `paymentMode=allinpay-url`，但 `sdkPayload` 仍固定透传 Java `/p/order/pay` 返回的完整 `data`。
...
当测试环境 `paySettlementType=1` 且用户选择微信支付 `payType=8` 时，H5 会先调用 Java `/p/order/pay`，将完整返回 `data` 放入 `sdkPayload`；当 `data.result == 0` 且 `data.chnlFrontParamInfo` 可解析时，H5 会将该 JSON 字符串解析为 `chnlFrontParamInfo` 对象并把对象内所有顶层参数传给原生，同时保留历史兼容辅助字段 `miniProgram`：
...
- `paymentMode=allinpay-mini-program-bridge` 时，App 不按普通微信支付 SDK 参数解析 `sdkPayload`；当前联调方案要求 App 以 `sdkPayload/chnlFrontParamInfo` 为准直接打开通联小程序收银台。
- `sdkPayload` 是 Java `/p/order/pay` 返回的完整 `data`；`chnlFrontParamInfo` 是 H5 从 `sdkPayload.chnlFrontParamInfo` 解析出的对象；对象内所有顶层字段都传给原生。`miniProgram.extraData.allinpayParams` 与该对象保持一致，仅用于兼容历史小程序桥页方案。
- 打开通联小程序收银台成功只表示已发起支付，不表示支付成功。H5 会在发起 Bridge 后立即进入 `/pay-result`，并通过 `/api/bff/order-is-paid?payEntry=0&orderNumbers=<orderNumbers>` 回查。若 App 仍返回结果，建议 resolve `{ "status": "unknown", "message": "已打开通联收银台" }`。
...
- `paymentStartAlipay/paymentStartWechat` 返回 `unsupported`：H5 自动重试旧 action `paymentStartCashier`。
- `paymentStartCashier` 也不可用：不调用 Java 支付成功逻辑，不伪造成功。
- `payment.openUrl` 不可用：Web 环境可尝试 `window.location.assign(url)`，App 环境展示错误；通联支付宝主链路优先走 `paymentStartAlipay`。
- 支付完成与否必须由 H5 回查订单状态。

## 原生实现要求

- [ ] 方法名保持稳定。
- [ ] 新增实现 `paymentStartAlipay` 和 `paymentStartWechat`，旧 `paymentStartCashier` 保留一个版本周期兼容。
- [ ] 校验 `provider/payType/orderNumbers/paymentUrl`。
- [ ] `paymentStartAlipay` 按 `paymentMode=app-sdk/allinpay-url` 分流，`sdkPayload` 始终完整透传。
- [ ] `paymentStartWechat` 按 `paymentMode=app-sdk/allinpay-mini-program-bridge` 分流，通联微信按 `sdkPayload/chnlFrontParamInfo` 直开通联小程序收银台。
