...
H5 钱包页需要展示分销员钱包余额、结算状态、推广订单和提现记录；银行卡管理页需要查询通联支付已绑银行卡并支持解绑。
...
| --- | --- | --- | --- |
| `GET` | `/api/bff/wallet/summary` | 钱包汇总 | `GET /p/distribution/wallet/info` |
...
| `GET` | `/api/bff/wallet` | 钱包汇总兼容入口 | `GET /p/distribution/wallet/info` |
...
- 新增字段：H5 忽略未知字段。
- 删除字段：删除 Cookie `userInfo.phone`、银行卡 `bankCardNo` 或解绑所需会员编号会影响功能，需提前通知 H5。
...
- H5 验证：打开 `/wallet`、`/wallet/withdraw-records` 和 `/wallet/bank-cards`，验证成功、空态、错误态、分页加载和解绑银行卡。
