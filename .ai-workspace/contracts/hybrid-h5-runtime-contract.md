...
喵呜当前一级 tab 包括首页、智能体、推广和我的。原生容器应优先采用“每个一级 tab 一个可复用 WebView”的模式。智能体 tab 由 App 原生负责，H5 侧不实现智能体具体业务。
...
二级页面包括商品详情、搜索结果、活动详情、订单确认、订单详情、支付、收藏、会员/达人和佣金等。二级页面可以在当前 tab 内路由跳转，也可以由原生页面栈 push 新 WebView，但必须保持一级 tab 的滚动位置和状态。
...
| 推广 | CSR 静态壳或 no-store SSR | 收益、佣金、达人等级等用户私有数据不缓存最终结果 |
| 我的 | CSR 静态壳或 no-store SSR | 不缓存用户 HTML，可展示弱摘要 |
| 订单确认、订单和支付 | CSR 或 no-store SSR | 不离线承诺，交易前必须实时确认 |
| 帮助和协议 | SSG | 可长期缓存 |
