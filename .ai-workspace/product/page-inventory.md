...
- 最近飞书同步：2026-07-06，revision_id=70，补充运营外部注册 H5 例外事实和 `/register` 固定公开入口；飞书表格内已可检索 `/register`。上一轮 2026-07-02 revision_id=65 更新活动中心真实 BFF、`displayStates` 状态分页、历史活动入口、活动详情按钮/奖励详情调用口径和活动规则入口。
...
- 项目是私域平台，App 内只有登录，没有注册；运营外部注册 H5 是例外入口，固定 URL 为 `https://hybird.aigcpop.com/register`。
- 登录后即是达人，不需要申请成为达人。
- 达人和会员是一套体系。
...
- 商品详情当前已接入普通商品 + 快递 + SKU + 立即购买 + 订单确认 + 普通快递订单创建 + 收银台真实发起支付；订单确认页加载阶段也会按旧 uni-app 参数调用 Java `/p/order/confirm` 生成确认上下文；秒杀、拼团、自提和同城后置。
- 首页首批真实接口已接入 H5 BFF，首页类目直接使用 Java `/p/app/home/index` 的 `navList`，不再拼接 `hotCategory/categoryTop8`；商品详情和订单确认首批真实链路已接入 H5 BFF；商品详情、订单确认和地址管理页的地址来源已升级为 App Bridge `rpc/address.*` 优先，H5 BFF/Java 地址接口兜底和服务端校验；地址管理已新增列表和新增/编辑页，并接入旧 Java 地址列表、保存、设默认和删除接口；商品详情/订单确认进入地址列表时已统一使用 `select/from/flowId` 地址选择流，选择地址通过一次性 `sessionStorage` 结果 + `history.back()` 回到来源页，兼容 App 导航栏返回和系统手势返回；收银台已接 `/p/order/pay`、`paySettlementType`、通联支付宝 URL 支付回查和通联微信小程序支付桥 Bridge payload，普通支付宝/微信分别通过 `rpc/paymentStartAlipay` / `rpc/paymentStartWechat` 交给 App SDK，旧 `paymentStartCashier` 仅作 fallback；订单列表、退货退款、普通订单详情、退款详情、我的收藏商品、我的足迹、钱包、账户管理认证信息和银行卡管理已接入真实 H5 BFF，待 App WebView 真实 `mallToken` 联调验证；个人中心优惠券仍为静态高保真或 mock。
...
| `/register` | 运营外部注册页 | 已接注册/认证流程，公开入口由 server-meumall 解析 | 注册短信验证码、用户注册、认证开户 H5、会员信息查询 | 外部 H5 / 新 WebView | 对外二维码固定使用 `https://hybird.aigcpop.com/register`；server-meumall 302 到 active H5 版本 `/h5-v/<version>/register`；这是运营外部注册例外，不是 App 内登录页注册入口 |
...
| `/order-confirm` | 订单确认 | 真实商品参数实时校验和普通快递订单创建已接入 | URL 地址优先；无地址时 Bridge `address.getDefault`；BFF `/api/bff/order-confirm` 校验地址/商品/SKU/库存/价格并调用 Java `/p/order/confirm` 生成确认上下文；`/api/bff/order-submit` 创建待支付订单 | 当前 WebView replace 到收银台 | 无收货地址时禁止提交；普通快递链路不以 Java 确认 `submitOrder=0` 置灰；提交成功后 replace 到 `/pay-way` |
| `/pay-way` | 收银台 | 真实支付发起链路已接入，待 App SDK/通联微信收银台联调 | BFF `/api/bff/order-pay-info` -> Java `/p/order/getOrderPayInfoByOrderNumber`、`/sys/config/info/getSysPaySwitch`、`/sys/config/paySettlementType`；确认付款 POST `/api/bff/order-pay` -> Java `/p/order/pay` | 当前 WebView replace 到结果页 | 普通支付宝/微信通过 `rpc/paymentStartAlipay/paymentStartWechat` 交给 App SDK，旧 App fallback 到 `paymentStartCashier`；测试环境 `paySettlementType=1` 时通联支付宝通过 `paymentStartAlipay` 传 `paymentUrl + sdkPayload`，通联微信通过 `paymentStartWechat` 传完整 `sdkPayload/chnlFrontParamInfo` 给 App 直开通联小程序收银台；H5 发起后立即 replace 到 `/pay-result` |
| `/pay-result` | 支付结果 | 已接订单号支付状态回查 | BFF `/api/bff/order-is-paid` -> Java `/p/order/isPay/{payEntry}/{orderNumbers}`；`payEntry=0` | 当前 WebView | 自动查询和手动“查看支付状态”都按订单号判断是否已支付；另提供“查看订单”进入订单详情 |
...
| `/wallet` | 钱包 | 已接真实 BFF，待 App token 联调 | BFF `/api/bff/wallet/summary` -> Java `/p/distribution/wallet/infoV2?userMobile=<userInfo.phone>`；BFF `/api/bff/wallet/history-status` -> Python `/user/wallet_state`；BFF `/api/bff/wallet/orders` -> Java `/p/distribution/api/queryPromotionOrder`；提现申请 `/api/bff/wallet/withdraw` -> `/p/allinpay/member/memberWithdrawApply` | 新 H5 WebView | 背景复用“我的”页同源图片 `mine.hero.background`；结算 tab 为页面内 state，不改 URL；钱包汇总 `userMobile` 和推广订单 `userId` 都来自原生 Cookie `userInfo.phone`；历史状态 `state=1` 时右上角展示“历史钱包”并发送 `router/navigate route=history-wallet`；账户余额、可提现金额和提现上限都取 `canWithdrawAmount`；切换 tab 只重置订单第一页，触底加载更多；提现弹窗只输入 `amount`；提现记录入口进入 `/wallet/withdraw-records`；帐户管理入口进入 `/wallet/account`；汇总接口失败时金额展示空占位，订单接口失败时展示订单空态，不回退 mock |
| `/wallet/account` | 账户管理 | 已接页面，待 App token 联调 | 本页不直接请求后端 | 当前 WebView push | 当前只展示“认证信息”入口，点击进入 `/wallet/account/certification` |
| `/wallet/account/certification` | 认证信息 | 已接真实 BFF，待 App token 联调 | BFF `/api/bff/wallet/member-info` -> Java `/p/allinpay/member/getMemberBasicInfoV2`；BFF `/api/bff/wallet/bank-cards` -> Java `/p/allinpay/member/queryBankCardV2` | 当前 WebView push | 展示会员姓名和身份证号；不展示“实名登记”；同时展示银行卡列表预览并可跳转 `/wallet/bank-cards` |
| `/wallet/withdraw-records` | 提现记录 | 已接真实 BFF，待 App token 联调 | BFF `/api/bff/wallet/withdraw-records` -> Java `/p/userWithdraw/pageDateUserWithdrawCash` | 新 H5 WebView | 按年月分组展示提现记录；支持触底加载更多、错误重试和空态 |
| `/wallet/bank-cards` | 银行卡管理 | 已接真实 BFF，待 App token 联调 | BFF `/api/bff/wallet/bank-cards` -> Java `/p/allinpay/member/queryBankCardV2`；添加 `/api/bff/wallet/bank-cards/apply` -> `/p/allinpay/member/createMemberApply`；解绑 `/api/bff/wallet/bank-cards/unbind` -> `/p/allinpay/member/unbindBankCardV2` | 当前或新 H5 WebView | 无卡态和列表底部均展示添加银行卡入口；添加成功返回本页并展示 toast；解绑确认弹窗按 Figma 节点 `681:31290`；添加只传 `acctNum/cerNum/phone`，解绑只传 `acctNum` |
| `/wallet/bank-cards/add` | 添加银行卡 | 已接真实 BFF，待 App token 联调 | BFF `/api/bff/wallet/bank-cards/apply` -> Java `/p/allinpay/member/createMemberApply` | 当前 WebView push | 表单只包含银行卡号、身份证号、手机号；不传 `signNum/name` |
...
| `/promotion/activities` | 活动中心 | 已接真实 BFF | BFF `/api/bff/promotion/activities` -> Java `/p/app/distribution/incentive/page` | 新 H5 WebView | 首屏分别传 `displayStates=[1,2,3,4]` 和 `[0]` 展示进行中/已暂停活动；底部进入历史活动；列表空数组展示空态，失败或 token 缺失不回退 mock |
| `/promotion/activities/history` | 历史活动 | 已接真实 BFF | BFF `/api/bff/promotion/activities` -> Java `/p/app/distribution/incentive/page` | 当前 WebView push | 传 `displayStates=[6]` 展示历史活动；无底部历史活动入口；支持空态和加载更多 |
| `/promotion/activities/[id]` | 活动详情 | 已接真实 BFF | BFF `/api/bff/promotion/activities/[id]` -> Java `/p/app/distribution/incentive/detail/{id}` | 当前 WebView push | 展示活动详情、个人进度和奖励规则；当前暂不聚合奖励详情，顶部不展示活动标题和 `ruleSummary`，按钮按 `displayState` 映射；右上角“活动规则”进入规则页；非法 id 404，失败不回退 mock |
| `/promotion/activities/[id]/rules` | 活动规则 | 已接真实 BFF | 复用 BFF `/api/bff/promotion/activities/[id]` -> Java `/p/app/distribution/incentive/detail/{id}` | 当前 WebView push | 展示 `ruleContent` 清洗后的富文本；空规则展示空态文案；失败不回退 mock |
...
| `/seller/activities` | 卖手营销活动 | 已接真实 BFF | BFF `/api/bff/seller-activities` -> Java `/p/sellerActivity/availableList` | 新 H5 WebView，由原生智能体入口打开 | 展示平台配置的可参加卖手活动；空数组展示空态，失败不回退 mock |
| `/seller/activities/[activityId]` | 卖手活动商品配置 | 已接真实 BFF | BFF `/api/bff/seller-activities/[activityId]/products` -> Java `/p/sellerActivity/page` | 当前 WebView push | 支持进行中/已暂停 tab，切换只更新页面 state 和接口请求，不改 URL；批量暂停/开始/删除 |
| `/seller/activities/[activityId]/products` | 新增活动商品选择 | 已接真实 BFF | BFF `/api/bff/seller-activities/[activityId]/available-products` -> Java `/p/distribution/prod/productPage` | 当前 WebView push | 使用推广商品分页作为商品来源，支持搜索、排序和进入商品设置 |
| `/seller/activities/[activityId]/products/[prodId]` | 卖手活动商品设置 | 已接真实 BFF | BFF `/api/bff/seller-activities/[activityId]/products/[prodId]` -> Java `/p/sellerActivity/detail`；保存走 `/api/bff/seller-activities/save-or-update` | 当前 WebView push | 支持限购、活动时间、SKU 活动价保存；保存后返回活动配置页 |
...
| `/api/bff/order-pay-info` | 收银台订单支付信息 | 已实现 | Java `/p/order/getOrderPayInfoByOrderNumber`、`/sys/config/info/getSysPaySwitch`、`/sys/config/paySettlementType` | 只读订单金额、过期时间、订单支付状态、可用支付方式和支付结算类型 |
| `/api/bff/order-pay` | 收银台确认付款 | 已实现，待 App SDK 联调 | Java `/sys/config/paySettlementType`、`/p/order/pay`、通联时 `/p/allinpay/order/getAliAppPayUrl` | POST `orderNumbers/payType`；`execution.bridgeAction` 按 `payType` 输出 `paymentStartAlipay/paymentStartWechat`，旧 App 可 fallback 到 `paymentStartCashier`；`sdkPayload` 透传 `/p/order/pay` 完整 `data`；通联支付宝返回 `paymentMode=allinpay-url + paymentUrl`，通联微信返回 `paymentMode=allinpay-mini-program-bridge + chnlFrontParamInfo` |
| `/api/bff/order-is-paid` | 订单支付状态确认 | 已实现，待真实订单联调 | Java `/p/order/isPay/{payEntry}/{orderNumbers}` | GET `payEntry=0&orderNumbers=<orderNumbers>`；支付结果页自动查询和手动刷新使用，Java `data=false` 作为合法未支付状态处理 |
| `/api/bff/allinpay-order-status` | 通联支付状态回查 | 历史兼容 | Java `/p/allinpay/order/getOrderStatus` | GET `bizOrderNo`；当前支付结果页主链路已改用 `/api/bff/order-is-paid` |
...
| `/api/bff/wallet/summary` | 钱包汇总 | 已实现，待 App token 联调 | Java `/p/distribution/wallet/infoV2` | 必传 `userMobile=userInfo.phone`；账户余额、可提现金额和提现上限统一取 `canWithdrawAmount`；旧 `/api/bff/wallet` 保留为汇总兼容入口 |
| `/api/bff/wallet/history-status` | 历史钱包状态 | 已实现，待 App token 联调 | Python `/user/wallet_state` | `state=1` 时钱包页展示“历史钱包”入口；点击发送原生路由 `history-wallet` |
| `/api/bff/wallet/orders` | 钱包推广订单分页 | 已实现，待 App token 联调 | Java `/p/distribution/api/queryPromotionOrder` | 支持 `state=settled/pending`、`current/size`；Cookie `userInfo.phone` 缺失或接口失败时页面展示订单空态，不回退 mock；页面触底加载下一页 |
| `/api/bff/wallet/withdraw-records` | 钱包提现记录分页 | 已实现，待 App token 联调 | Java `/p/userWithdraw/pageDateUserWithdrawCash` | 支持 `current/size`；Java 返回 `date + withdrawCashVOs` 年月分组，H5 保留分组展示并触底加载下一页 |
| `/api/bff/wallet/withdraw` | 钱包提现申请 | 已实现，待 App token 联调 | Java `/p/allinpay/member/memberWithdrawApply` | POST `{ amount }`；BFF 提交前读取 `/p/distribution/wallet/infoV2?userMobile=<userInfo.phone>`，校验金额大于 0 且不超过 `canWithdrawAmount`；不传 `signNum/notifyUrl` |
| `/api/bff/wallet/member-info` | 会员认证信息 | 已实现，待 App token 联调 | Java `/p/allinpay/member/getMemberBasicInfoV2` | 认证信息页使用；展示 `memberName/name` 和脱敏 `cerNum` |
| `/api/bff/wallet/bank-cards` | 银行卡列表 | 已实现，待 App token 联调 | Java `/p/allinpay/member/queryBankCardV2` | 过滤已解除卡 |
| `/api/bff/wallet/bank-cards/apply` | 添加银行卡 / 创建会员申请 | 已实现，待 App token 联调 | Java `/p/allinpay/member/createMemberApply` | POST `{ acctNum, cerNum, phone }`；不传 `signNum/name`；成功后返回银行卡管理并提示 |
| `/api/bff/wallet/bank-cards/unbind` | 解绑银行卡 | 已实现，待 App token 联调 | Java `/p/allinpay/member/unbindBankCardV2` | POST `{ acctNum }`；不传 `signNum`；成功后页面刷新银行卡列表 |
...
| `/api/bff/promotion/activities` | 推广激励活动列表 | 已实现 | Java `/p/app/distribution/incentive/page` | `/promotion/activities` 和 `/promotion/activities/history` 使用；支持 `current/size/orderBy/displayStates`；活动中心分 `[1,2,3,4]`、`[0]` 两次查询，历史活动传 `[6]`；空数据展示空态，失败不回退 mock |
| `/api/bff/promotion/activities/[id]` | 推广激励活动详情 | 已实现 | Java `/p/app/distribution/incentive/detail/{id}` | `/promotion/activities/[id]` 和 `/promotion/activities/[id]/rules` 使用；当前详情页只展示活动进度和奖励规则，未拿到奖励详情时隐藏“我的奖励”区块；规则页展示 `ruleContent` |
| `/api/bff/promotion/activities/[id]/reward` | 推广激励奖励详情 | 已实现 | Java `/p/app/distribution/incentive/reward/detail/{id}` | 独立保留给后续领取/刷新奖励状态使用，活动详情页当前不主动调用 |
| `/api/bff/promotion/activities/rewards/[recordId]/receive` | 领取推广激励奖励 | 已实现 | Java `PATCH /p/app/distribution/incentive/reward/receive/{recordId}` | 支持可选 `addressId`；实物奖励地址选择完整交互后置 |
...
| `/api/bff/seller-activities` | 卖手营销活动入口 | 已实现 | Java `/p/sellerActivity/availableList` | `/seller/activities` 使用；失败或 token 缺失不回退 mock |
| `/api/bff/seller-activities/[activityId]/products` | 卖手活动商品分页 | 已实现 | Java `/p/sellerActivity/page` | 支持 `status=1/0`、`current/size`；用于活动配置页进行中/已暂停 tab |
| `/api/bff/seller-activities/[activityId]/available-products` | 卖手活动可选商品 | 已实现 | Java `/p/distribution/prod/productPage` | 支持 `keyword/categoryId/orderBy/incentiveId/current/size`；用于新增活动商品 |
| `/api/bff/seller-activities/[activityId]/products/[prodId]` | 卖手活动商品详情 | 已实现 | Java `/p/sellerActivity/detail` | 商品设置页回填；空 data 时允许新增模式兜底 |
| `/api/bff/seller-activities/save-or-update` | 新增或修改卖手活动商品 | 已实现 | Java `/p/sellerActivity/saveOrUpdate` | POST 活动、商品、限购、时间和 SKU 活动价 |
| `/api/bff/seller-activities/batch-status` | 批量更新卖手活动商品状态 | 已实现 | Java `/p/sellerActivity/batchStatus` | `status=-1` 删除，`0` 暂停，`1` 开始 |
...
| 登录 | 手机号/验证码或授权登录 | 未登录访问受限功能 | 首页、上一页 | App 优先 | 否 | 凭证只能进安全存储 | 私域平台 App 内只有登录，没有注册；运营外部注册 H5 为单独投放入口 |
