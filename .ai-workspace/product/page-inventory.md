...
- 最近飞书同步：2026-07-01，revision_id=48，新增“钱包提现记录页真实接口”同步记录。
...
| `/wallet` | 钱包 | 已接真实 BFF，待 App token 联调 | BFF `/api/bff/wallet/summary` -> Java `/p/distribution/wallet/info`；BFF `/api/bff/wallet/orders` -> Java `/p/distribution/api/queryPromotionOrder` | 新 H5 WebView | 结算 tab 为页面内 state，不改 URL；推广订单 `userId` 来自原生 Cookie `userInfo.phone`；切换 tab 只重置订单第一页，触底加载更多；提现记录入口进入 `/wallet/withdraw-records`；接口失败/空数据不回退 mock |
...
| `/wallet/bank-cards` | 银行卡管理 | 已接真实 BFF，待 App token 联调 | BFF `/api/bff/wallet/bank-cards` -> Java `/p/allinpay/member/queryBankCardV2`；解绑 `/api/bff/wallet/bank-cards/unbind` -> `/p/allinpay/member/unbindBankCardV2` | 当前或新 H5 WebView | 无卡态展示添加银行卡入口但新增绑卡后置；解绑确认弹窗按 Figma 节点 `681:31290`；`signNum` 暂取推广概览 `userInfo.cardNo`，需后端联调确认 |
...
| `/api/bff/wallet/summary` | 钱包汇总 | 已实现，待 App token 联调 | Java `/p/distribution/wallet/info` | 只读钱包金额；旧 `/api/bff/wallet` 保留为汇总兼容入口 |
| `/api/bff/wallet/orders` | 钱包推广订单分页 | 已实现，待 App token 联调 | Java `/p/distribution/api/queryPromotionOrder` | 支持 `state=settled/pending`、`current/size`；Cookie `userInfo.phone` 缺失时返回错误态，不回退 mock；页面触底加载下一页 |
...
| `/api/bff/wallet/bank-cards` | 银行卡列表 | 已实现，待 App token 联调 | Java `/p/distribution/home/overview`、`/p/allinpay/member/queryBankCardV2` | 过滤已解除卡；返回 `signNum` 供解绑使用 |
| `/api/bff/wallet/bank-cards/unbind` | 解绑银行卡 | 已实现，待 App token 联调 | Java `/p/allinpay/member/unbindBankCardV2` | POST `{ signNum, acctNum }`；成功后页面刷新银行卡列表 |
