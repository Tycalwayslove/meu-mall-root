...
- 最近飞书同步：2026-07-01，revision_id=54，新增“钱包账户管理与认证信息页”同步记录。
...
| `/pay-way` | 收银台 | 真实支付发起链路已接入，待 App SDK/微信小程序支付桥联调 | BFF `/api/bff/order-pay-info` -> Java `/p/order/getOrderPayInfoByOrderNumber`、`/sys/config/info/getSysPaySwitch`、`/sys/config/paySettlementType`；确认付款 POST `/api/bff/order-pay` -> Java `/p/order/pay` | 当前 WebView push | 普通支付宝/微信通过 `rpc/paymentStartCashier` 交给 App SDK；测试环境 `paySettlementType=1` 时通联支付宝通过 `rpc/payment.openUrl` 打开支付 URL，通联微信通过 `rpc/paymentStartCashier` 的 `paymentMode=allinpay-mini-program-bridge` 交给 App 打开喵呜小程序支付桥页，并跳 `/pay-result` 回查 |
| `/pay-result` | 支付结果 | 已接通联回查和支付信息刷新 | BFF `/api/bff/allinpay-order-status` -> Java `/p/allinpay/order/getOrderStatus`；非通联时回读 `/api/bff/order-pay-info` | 当前 WebView push | 支持成功、失败、待确认、重试付款和查看订单；确认付款 App SDK 正式回调仍需 iOS/Android 接入 |
...
| `/promotion/activities` | 活动中心 | 已接真实 BFF | BFF `/api/bff/promotion/activities` -> Java `/p/app/distribution/incentive/page` | 新 H5 WebView | 展示当前达人可参与或待领奖激励活动；列表空数组展示空态，失败或 token 缺失不回退 mock |
| `/promotion/activities/[id]` | 活动详情 | 已接真实 BFF | BFF `/api/bff/promotion/activities/[id]` -> Java `/p/app/distribution/incentive/detail/{id}` + `/p/app/distribution/incentive/reward/detail/{id}` | 当前 WebView push | 展示活动详情、个人进度、奖励规则和奖励状态；非法 id 404，失败不回退 mock |
...
| `/api/bff/order-pay` | 收银台确认付款 | 已实现，待 App SDK 联调 | Java `/sys/config/paySettlementType`、`/p/order/pay`、通联时 `/p/allinpay/order/getAliAppPayUrl` | POST `orderNumbers/payType`；`paymentStartCashier.sdkPayload` 透传 `/p/order/pay` 完整 `data`，通联支付宝返回外部支付 URL，通联微信返回喵呜小程序支付桥参数 |
| `/api/bff/allinpay-order-status` | 通联支付状态回查 | 已实现，待真实订单联调 | Java `/p/allinpay/order/getOrderStatus` | GET `bizOrderNo`，支付结果页轮询/手动刷新使用 |
...
| `/api/bff/promotion/activities` | 推广激励活动列表 | 已实现 | Java `/p/app/distribution/incentive/page` | `/promotion/activities` 使用；支持 `current/size/orderBy`；空数据展示空态，失败不回退 mock |
| `/api/bff/promotion/activities/[id]` | 推广激励活动详情 | 已实现 | Java `/p/app/distribution/incentive/detail/{id}`、`/p/app/distribution/incentive/reward/detail/{id}` | `/promotion/activities/[id]` 使用；展示活动进度、奖励节点和奖励状态 |
| `/api/bff/promotion/activities/[id]/reward` | 推广激励奖励详情 | 已实现 | Java `/p/app/distribution/incentive/reward/detail/{id}` | 后续领取/刷新奖励状态使用 |
