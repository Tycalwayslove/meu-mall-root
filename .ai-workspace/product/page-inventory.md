...
- 最近飞书同步：2026-06-30，revision_id=41，新增“钱包与银行卡真实接口联调”同步记录。
...
| `/pay-way` | 收银台 | 真实支付发起链路已接入，待 App SDK 联调 | BFF `/api/bff/order-pay-info` -> Java `/p/order/getOrderPayInfoByOrderNumber`、`/sys/config/info/getSysPaySwitch`、`/sys/config/paySettlementType`；确认付款 POST `/api/bff/order-pay` -> Java `/p/order/pay` | 当前 WebView push | 普通支付宝/微信通过 `rpc/payment.pay` 交给 App SDK；测试环境 `paySettlementType=1` 时通联支付宝通过 `rpc/payment.openUrl` 打开支付 URL，并跳 `/pay-result` 回查 |
