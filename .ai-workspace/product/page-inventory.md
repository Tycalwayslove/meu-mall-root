...
- 最近飞书同步：2026-06-30，revision_id=42，新增“收银台通联微信小程序收银台 Bridge”同步记录。
...
| `/pay-way` | 收银台 | 真实支付发起链路已接入，待 App SDK/微信小程序收银台联调 | BFF `/api/bff/order-pay-info` -> Java `/p/order/getOrderPayInfoByOrderNumber`、`/sys/config/info/getSysPaySwitch`、`/sys/config/paySettlementType`；确认付款 POST `/api/bff/order-pay` -> Java `/p/order/pay` | 当前 WebView push | 普通支付宝/微信通过 `rpc/payment.pay` 交给 App SDK；测试环境 `paySettlementType=1` 时通联支付宝通过 `rpc/payment.openUrl` 打开支付 URL，通联微信通过 `rpc/payment.pay` 的 `paymentMode=wechat-mini-program` 交给 App 打开微信小程序收银台，并跳 `/pay-result` 回查 |
...
| `/wallet` | 钱包 | 已接真实 BFF，待 App token 联调 | BFF `/api/bff/wallet` -> Java `/p/distribution/wallet/info`、`/p/distribution/home/overview`、`/p/distribution/api/queryPromotionOrder` | 新 H5 WebView | 结算 tab 为页面内 state，不改 URL；推广订单 `userId` 来自推广概览 `userInfo.distributionUserId`；接口失败/空数据不回退 mock |
...
| `/api/bff/order-pay` | 收银台确认付款 | 已实现，待 App SDK 联调 | Java `/sys/config/paySettlementType`、`/p/order/pay`、通联时 `/p/allinpay/order/getAliAppPayUrl` | POST `orderNumbers/payType`；普通支付宝/微信返回 App SDK payload，通联支付宝返回外部支付 URL |
...
| `/api/bff/wallet` | 钱包汇总与推广订单 | 已实现，待 App token 联调 | Java `/p/distribution/wallet/info`、`/p/distribution/home/overview`、`/p/distribution/api/queryPromotionOrder` | 支持 `state=settled/pending`、`current/size`；`distributionUserId` 缺失时返回错误态，不回退 mock |
